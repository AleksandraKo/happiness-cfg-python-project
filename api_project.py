...
def run():
    ingredient = input('Enter the key ingredient you want in your dish: ')
    ...
    if mealType in ['lunch','dinner']:
         ...
         if diet == 'yes':
             recipes = another_search(diet) and recipe_search(mealType) and recipe_search(ingredient)
            ...
            recipes_0=  recipe_search(mealType) and recipe_search(ingredient)
        ...
        recipes1 = recipe_search(mealType) and recipe_search(ingredient)
    ...
    recipes3 = recipe_search(mealType) and recipe_search(ingredient)
    ...
    sat = input('Are you satisfied with these selections?')
    if sat == 'yes':
        print("Enjoy your dish!")

    else:
        health = input('Do you need to know options which are more suitable for those with dietary restrictions?')
        if health == 'yes':
            recipes2 = recipe_search(mealType) and recipe_search(ingredient) and diet_check(health)
